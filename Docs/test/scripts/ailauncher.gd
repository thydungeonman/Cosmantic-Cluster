...
#onready var midwallpos = get_parent().get_node("middlewall").get_position()
var bottomorbs = []
...
func _ready():
	...
	add_child(next)
	next.set_position(Vector2(0,20))
	AimReticule()
...
#		state = 0
#		clickedpos = get_global_mouse_position()
#		print("clicked pos = " + str(clickedpos - get_position()))
#		aiming = true
#		#CheckAim(clickedpos)
...
#			CheckAim(clickedpos)
	if(state == 2):
		aiming = AimAtPos(clickedpos - get_position())
		if(aiming):
			state = 3
	if(state == 3):
		Fire()
...
func Click():
	...
	if(Input.is_action_pressed("click")):
		clicked = true
		return
		var shot = get_global_mouse_position()
		var goodshot = false
		var lchecker = preload("res://test/scenes/laserchecker.tscn").instance()
		add_child(lchecker)
		while goodshot == false:
			var difference = shot - lchecker.get_global_position()
			lchecker.set_rotation(0)
			lchecker.look_at(lchecker.get_global_position() + difference)
			var trajectory = shot - get_global_position()
...
#			print(str(y) + " = " + str(m) + " * " + str(s) + "+ 980" )
			...
			add_child(lchecker2)
			lchecker2.set_global_position(Vector2(x,y))
...
#		randomize()
	...
	if(shottimer > 1):
		if(loaded == false):
			...
			orb.set_position(get_global_position())#set the orb to launcher position
...
#		if(storing == false):
#			orb.set_position(Vector2(0,-200)) #move the orb to the ether else it stays in the same spot and collides with new orbs
#			get_parent().remove_child(orb)
#			print(str(get_parent().orbsonboard.size()))
#			#get_parent().orbsonboard.remove_and_collide(get_parent().orbsonboard.find(orb))
#			print(str(get_parent().orbsonboard.size()))
...
#			get_parent().add_child(orb)
#			orb.set_position(get_global_position())
#			#get_parent().orbsonboard.push_front(orb)
#			swapping = true
#			swapped = true
...
func Fire():
	...
	if(laserisactive):
		...
		orb.get_node("sparkles").set_rotation(x- (PI/2))
		orb.ismoving = true
...
#		randomize()
		...
		var localpos = clickedpos - get_position()
		...
		centercast.set_rotation(centercast.get_position().angle_to_point(clickedpos - get_position()))
...
func ThrowAway2(straightshottargets,bounceshottargets,warpshottargets,emptyshots):
	...
	for target in straightshottargets.keys():
		...
		if(target.isflag):
			continue
		if(get_position().distance_to(target.get_position()) < 130):
	...
	for target in bounceshottargets.keys():
		...
		if(target.isflag):
			continue
		if(get_position().distance_to(target.get_position()) < 200):
	...
	for target in warpshottargets.keys():
		...
		if(target.isflag):
			continue
		if(get_position().distance_to(target.get_position()) < 160):
			print("TOO CLOSE")
			continue
		potentialtargets.push_back(target)
...
#		clickedpos = fulltargetdict[potentialtargets[shot]]
		...
		for shot in range(potentialtargets.size()):
			if(potentialtargets[shot].get_position().y < potentialtargets[highest].get_position().y):
				highest = shot
...
#	print(str(oldorb))
#	print(GetStoredOrbs())
	oldorb.set_position(Vector2(0,-200)) #move the orb to the ether else it stays in the same spot and collides with new orbs
	get_parent().remove_child(oldorb)
	#get_parent().orbsonboard.remove_and_collide(get_parent().orbsonboard.find(orb))
	var neworb = container.Swap(oldorb)
	get_parent().add_child(neworb)
	neworb.set_position(get_global_position())
...
func Store(orb):
	if(!container.IsFull()):
		orb.set_position(Vector2(0,-200)) #move the orb to the ether else it doesn't stay in the same spot and collide with new orbs
		get_parent().remove_child(orb)
		container.TakeOrb(orb)
		loaded = false
		storing = true
...
func CheckBottomLayer():
	...
	if(borbs.size() == 1):
		print("ONE BORB")
		clickedpos = borbs[0].get_position()
		foundtarget = true
		state = 1
		return borbs[0]
	elif(borbs.size() > 1):
...
func CheckAim(position):
	var localpos = position - get_position()
	var hyp = sqrt(localpos.x * localpos.x + localpos.y * localpos.y) * -1
	var hypvector = Vector2(0,hyp)
	centercast.set_cast_to(hypvector)
	leftcast.set_cast_to(hypvector)
	rightcast.set_cast_to(hypvector)
	centercast.set_rotation(centercast.get_position().angle_to_point(position - get_position()))
	centercast.force_raycast_update()
	rightcast.force_raycast_update()
	leftcast.force_raycast_update()
	...
	if(localpos.x <= 0): # left side shot
		if(!centercast.is_colliding() and !rightcast.is_colliding()):
			Mirror(target.get_global_position(),0)
			print("BOUNCE SHOT")
				#ThrowAway()
				#aiming = false
			state = 2
...
#				print("ADJUST LEFT")
		...
		if(!centercast.is_colliding() and !leftcast.is_colliding()):
			Mirror(target.get_global_position(),1)
			print("BOUNCE SHOT")
			aiming = false
			state = 2
			return
...
func CheckFlagShot():
	var localpos = playerflagpos - get_position()
	var hyp = sqrt(localpos.x * localpos.x + localpos.y * localpos.y) * -1
	var hypvector = Vector2(0,hyp)
	centercast.set_cast_to(hypvector)
	leftcast.set_cast_to(hypvector)
	rightcast.set_cast_to(hypvector)
	centercast.set_rotation(centercast.get_position().angle_to_point(playerflagpos - get_position()))
	centercast.force_raycast_update()
	rightcast.force_raycast_update()
	leftcast.force_raycast_update()
...
#	add_child(g)
#	rgodots.push_back(g)
#	g.set_position(scanner.get_position())
	var hits = []
	var lastshotorb 
	while (i != 120):
		var remainder = scanner.move_and_collide(currentpoint)
		if(scanner.is_colliding()):
			if(scanner.get_collider().is_in_group("orb")):
				...
				if currentorb != lastorb:
					...
					hits = [scanner.get_global_position()]
					pointdict[currentorb] = scanner.get_global_position()
					lastorb = currentorb
				else:
					hits.push_back(scanner.get_global_position())
				list.push_back(scanner.get_collider())
#				scanner.get_collider().get_node("Sprite").modulate.a = 0
				var spot = scanner.get_global_position()
			
			if(scanner.get_collider().is_in_group("wall")):
				bouncedict[scanner.get_global_position()] = remainder
		else:
			emptyshots.push_back(currentpoint)
		scanner.set_position(Vector2())
	...
	for key in bouncedict.keys():
		bouncescanner.set_global_position(key)
		bouncescanner.move_and_collide(Vector2(-bouncedict[key].x,bouncedict[key].y))
...
#				print(bouncescanner.get_collider().get_name())
			...
			if(m > 0):
				...
				if(realx < 1860): #1868
					pass
					warpbouncedict[bouncedict[key]] = [Vector2(1920 - realx,0),key]
		bouncescanner.set_position(Vector2())
	...
	for shot in emptyshots:
		...
		var realx = localx + get_position().x
		scanner.set_global_position(Vector2(0 + (1920 - realx),0))
		scanner.move_and_collide(-shot)
		if(scanner.is_colliding() and scanner.get_collider().is_in_group("orb")):
			warppointdict[scanner.get_collider()] = shot + get_position()

	for key in warpbouncedict.keys():
		scanner.set_global_position(warpbouncedict[key][0])
		scanner.move_and_collide(Vector2(key.x,-key.y))
		if(scanner.is_colliding()):
			if(scanner.get_collider().is_in_group("orb")):
		...
		if(borbs.size() == 1):
			...
			if(borbs[0].colour == aiflagcolour):
				...
				if borbs[0].SearchFor(7,borbs[0].colour,get_parent().p2flag,true) or borbs[0] == get_parent().p2flag:
					bail1 = true 
					fulltargetlist.remove_and_collide(fulltargetlist.find(borbs[0]))
...
#						bail2 = true
#						print("BAILING !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
#						fulltargetlist.remove_and_collide(fulltargetlist.find(i))
#						break
				if(borb.colour == get_parent().p2flag.colour):
					if(borb.SearchFor(7,borb.colour,get_parent().p2flag,true)):
						fulltargetlist.remove_and_collide(fulltargetlist.find(borb))
