...
func Move(delta):
	...
	move_and_collide(trajectory * delta)
	if(get_position().y > 1080 or get_position().y < -400):
		#get_parent().orbsonboard.remove_and_collide(get_parent().orbsonboard.find(self))
		EnableLauncher()
		queue_free()
	...
	if(is_colliding()):
		...
		if(collider.is_in_group("wall")):
			...
			if(!collider.ismoving and !collider.inlauncher):
				...
				ismoving = false
				#print("playing sfx")
				var positiondifference = get_position() - collider.get_position()
...
#				print(dotproductwest)
				...
				if((dotproductnorth <= -.33 and dotproductnorth >= -1) and (dotproductwest <= 1 and dotproductwest > 0)): #collided on bottomleft
					set_position(collider.bottomleftspot)
					...
					set_position(collider.toprightspot)
				...
				if(foundmatch):
					...
					for orb in leftoverorbs:
						if(orb.colour == COLOUR.GREY):
							orb.TakeDamage()
					if(charged):
						get_parent().get_node("lightningarea").set_position(get_global_position())
						...
						for orb in matchingorbs:  #remove the original match orbs
							killorbs.remove_and_collide(killorbs.find(orb))
						for orb in greyorbs:
#							print(orb)
							orb.TakeDamage()
						for orb in killorbs:
						...
						for korb in killorbs:
							extraleftovers.remove_and_collide(extraleftovers.find(korb))
						for orb in extraleftovers:
							if(!leftoverorbs.has(orb)):
								leftoverorbs.push_back(orb)
					for i in matchingorbs:
						i.Unhook()
						get_parent().orbsonboard.remove_and_collide(get_parent().orbsonboard.find(i))
						i.RemoveFromPlayer()
...
#	ray.set_cast_to(mousepos - get_position())
#	print(ray.get_collider())
	#var positiondifference = mousepos - get_position()
	#positiondifference = positiondifference.normalized()
	#var dotproductnorth = positiondifference.dot(Vector2(0,-1))
	#var dotproductwest = positiondifference.dot(Vector2(-1,0))
...
func Die():
	...
	if(index != -1):
		get_parent().orbsonboard.remove_and_collide(index)
	var leftovers = []
	Search(2,COLOUR.NONE,leftovers)
	if(leftovers.size() > 0):
		leftovers.remove_and_collide(leftovers.find(self))
...
#	dyingsparks.set_global_position(get_global_position())
...
func RemoveFromPlayer():
	if(onboard == PLAYER.PLAYER1):
		...
		if(index != -1):
			get_parent().orbsonboardp1.remove_and_collide(index)
	elif(onboard == PLAYER.PLAYER2):
		var index = get_parent().orbsonboardp2.find(self)
		if(index != -1):
			get_parent().orbsonboardp2.remove_and_collide(index)
#	elif(onboard == PLAYER.AI):
#		var index = get_parent().orbsonboardai.find(self)
#		if(index != -1):
#			get_parent().orbsonboardai.remove_and_collide(index)
...
func PrintNeighbors():
	print(str(get_global_position()))
	print(str(get_position() - get_parent().get_node("p2launcher").get_global_position()))
...
func DoCasts():
	...
	if(ray.is_colliding() and ray.get_collider() != self and !ray.get_collider().is_in_group("warp")):
		...
		if(ray.get_collider().is_in_group("top")):
			istouchingtop = true
			#ray.get_collider().modulate.a = ray.get_collider().modulate.a - .15
...
func WentOverDeathLine(): # check to see if an orb is lower than y = 1000, which would put it right next to the launcher
	return get_global_position().y >= 930
...
func spawnFallOrb():
	...
	fallorb.set_global_position(get_global_position())
	fallorb.changeColour(colour)
	fallorb.setRot(get_node("Sprite").get_rotation())
